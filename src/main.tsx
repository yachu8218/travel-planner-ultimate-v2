...
import {Plus,Pencil,Copy,Trash2,ArrowLeft,Share2,FileDown,Upload} from 'lucide-react'
import './styles.css'

type TType='place'|'meal'|'hotel'|'transport'|'flight'|'note'
type Item={id:string,type:TType,start:string,end:string,title:string,note?:string,checks?:{id:string,text:string,done:boolean}[]}
type Day={id:string,date:string,title:string,items:Item[]}
type Trip={id:string,name:string,destination:string,country:string,currency:string,language:string,locale:string,start:string,end:string,lat:number,lon:number,cover?:string,days:Day[],theme:string,created:number,updated:number}
...
const makeTrip=(v:any):Trip=>{const p=profile(v.destination);return{id:id(),name:v.name||`${v.destination}旅行`,destination:v.destination,...p,start:v.start,end:v.end,cover:v.cover,theme:'summer',days:range(v.start,v.end),created:Date.now(),updated:Date.now()}}
const load=():State=>{try{const s=JSON.parse(localStorage.getItem(KEY)||'null');if(s?.version===2)return s}catch{}return{version:2,active:null,trips:[]}}
...
const shared=(()=>{const q=new URLSearchParams(location.search).get('trip');if(!q)return null;try{return JSON.parse(ub64(q)) as Trip}catch{return null}})()
...
function Form({trip,onSave,onClose}:{trip?:Trip,onSave:(v:any)=>void,onClose:()=>void}){
 const [v,setV]=useState({name:trip?.name||'',destination:trip?.destination||'',start:trip?.start||'',end:trip?.end||'',cover:trip?.cover||''})
...
function ItemForm({onSave,onClose}:{onSave:(x:Item)=>void,onClose:()=>void}){
 const [v,setV]=useState({type:'place' as TType,start:'09:00',end:'10:00',title:'',note:''})
 return <div className="shade"><form className="modal card" onSubmit={e=>{e.preventDefault();onSave({id:id(),...v,checks:v.type==='note'?v.note.split('\n').filter(Boolean).map(text=>({id:id(),text,done:false})):undefined})}}>
  ...
  <label>標題<input required value={v.title} onChange={e=>setV({...v,title:e.target.value})}/></label>
...
function App(){
 ...
 const saveTrip=(v:any)=>{if(form&&form!==true){const p=profile(v.destination);const t={...form,...v,...p,updated:Date.now()};update({...s,trips:s.trips.map(x=>x.id===t.id?t:x)})}else{const t=makeTrip(v);update({...s,active:t.id,trips:[...s.trips,t]})}setForm(null)}
 ...
 const duplicate=(t:Trip)=>{const c={...structuredClone(t),id:id(),name:t.name+'（複製）',created:Date.now(),updated:Date.now()};update({...s,active:c.id,trips:[...s.trips,c]})}
 ...
 if(active)return <div className="app">
  ...
  <main className="content trip">
   ...
   {active.days.map(d=><section className="card day" key={d.id}><div className="day-head"><div><small>{new Date(d.date+'T12:00:00').toLocaleDateString('zh-TW',{weekday:'long'})}</small><h2>{d.title}・{d.date.slice(5)}</h2></div>{!readOnly&&<button className="icon" onClick={()=>setItemDay(d.id)}><Plus/></button>}</div>
    <div className="timeline">{d.items.length?d.items.sort((a,b)=>a.start.localeCompare(b.start)).map(i=><article className={`item ${i.type}`} key={i.id}><div className="time">{i.start}<span>～</span>{i.end}</div><div className="body"><small>{i.type}</small><h3>{i.title}</h3>{i.note&&i.type!=='note'&&<p>{i.note}</p>}{i.checks&&<div className="checks">{i.checks.map(c=><label key={c.id}><input disabled={readOnly} type="checkbox" checked={c.done} onChange={()=>toggle(d.id,i.id,c.id)}/><span>{c.text}</span></label>)}</div>}</div></article>):<p className="empty">還沒有行程，按右上角＋加入。</p>}</div>
   </section>)}
  </main>{form&&<Form trip={form===true?undefined:form} onSave={saveTrip} onClose={()=>setForm(null)}/>} {itemDay&&<ItemForm onSave={addItem} onClose={()=>setItemDay(null)}/>}
 </div>
 return <div className="app"><header className="dash-head"><span className="stamp">ULTIMATE 2.0</span><h1>我的旅行手帳</h1><p>把每一次出發，收進自己的旅行書架。</p></header><main className="content"><div className="section"><div><small>MY JOURNEYS</small><h2>旅行書架</h2></div><button className="btn primary" onClick={()=>setForm(true)}><Plus size={18}/>新增旅行</button></div>
 {s.trips.length?<div className="grid">{s.trips.map(t=><article className="trip-card" key={t.id}><button className="trip-cover" style={t.cover?{backgroundImage:`url(${t.cover})`}:{}} onClick={()=>update({...s,active:t.id})}><div><small>{t.country}</small><b>{t.destination}</b><span>{t.start} ～ {t.end}</span></div></button><div className="trip-info"><div><h3>{t.name}</h3><p>{t.currency}・{t.language}</p></div><div className="icons"><button onClick={()=>setForm(t)}><Pencil size={17}/></button><button onClick={()=>duplicate(t)}><Copy size={17}/></button><button onClick={()=>remove(t)}><Trash2 size={17}/></button></div></div></article>)}</div>:<section className="card empty-home"><div>🧳</div><h2>建立第一本旅行手帳</h2><button className="btn primary" onClick={()=>setForm(true)}><Plus/>新增旅行</button><button className="btn" onClick={legacy}><Upload size={17}/>匯入舊版</button></section>}</main>{form&&<Form trip={form===true?undefined:form} onSave={saveTrip} onClose={()=>setForm(null)}/>}</div>
}
